...
1. 运行 `python3 tools/validate-tasks.py <tasks JSON>` 和 `python3 tools/check-traceability.py`；失败立即停止。
...
- 使用 Maven 标准布局：`workspace/src/main/java/<base-package>`、`workspace/src/main/resources`、`workspace/src/test/java`。
- 先读取 `docs/backend-project-profile.yml`，以 profile 中的 `workspace` 路径、package 和 Maven 入口为准。
...
拓扑分层执行；无依赖且不写同一文件的任务并行，主 agent 串行收口共享文件、任务 status 和校验。每批至少运行 `./tools/backend.sh validate` 或在未接入 workspace 时明确无法执行。
